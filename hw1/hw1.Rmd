...
$$
\begin{aligned}
Y &= \log X_{t,m}^{\text{obs}} - \log Y_t, \\
\gamma &= \log \alpha \cdot \mathbf{1} = \log \alpha_m \cdot 1,\ \text{and} \\
P &= \log P_t - \log P_{t,m};
\end{aligned}
$$

let $B = \left( \mathbf{1}, P \right)$. Thus, we can estimate

$$
\binom{\log \hat{\alpha}}{\hat{\sigma}} = \left( B^T B \right)^{-1} B^T Y.
$$

Plus, with our predictions $\displaystyle \log X_{t,m}^{\text{pred}} = B \cdot \binom{\log \hat{\alpha}}{\hat{\sigma}} + \log Y_t$, import wedges were defined as follows:

$$
\tau_t = \log X_{t,m}^{\text{obs}} - \log X_{t,m}^{\text{pred}}.
$$
